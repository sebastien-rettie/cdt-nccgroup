...
class DGCNN(nn.Module):
    def __init__(self, num_features, num_classes):
        ...
        self.conv3 = GCNConv(32, 32) # These layers are automatically adding self loops and normalising,
    ...
    def forward(self, data):
        ...
        x_4 = torch.tanh(self.conv4(x_3, edge_index)) # Could try removing one graph conv and removing the skip connection -- then could have more channels
        x = torch.cat([x_1, x_2, x_3, x_4], dim=-1)
        x = global_sort_pool(x, batch, k=30) # Can try adaptive pooling
        x = x.view(x.size(0), 1, x.size(-1))
        x = self.relu(self.conv5(x)) # Could try 2d convolutions, could also try without the 1d convs at all
        x = self.pool(x)
