...
def main(train_dir, valid_dir, epochs):
    ...
    model = DGCNN(303, 12) # 12 for cg, 13 for cfg
    model.to(DEVICE)
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            for batch_idx, batch_files in enumerate(graphs_for_batches): # Training for one batch
                ...
                for probs in out.detach().tolist():
        ...
        for i in range(12):
            correct = len([ 1 for pred, true in zip(y_pred, y_true) if (pred == true & true == i) ])
            tot = len([ 1 for pred, true in zip(y_pred, y_true) if true == i ])
            print("{}: {}".format(i, float(correct)/float(tot)))
