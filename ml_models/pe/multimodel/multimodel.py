...
for name, model in zip(names, classifiers):
    ...
    try:
        ...
        print('Roc plot failed for model {0}.\n'.format(name))
        fails.append((name, 'ROC'))
    ...
    try:
        ...
        print('Confusion matrix plot failed for model {0}.\n'.format(name))
        fails.append((name, 'Confusion matrix'))
    ...
    print("Plotting roc (second)")
    try:
        model_displays[name] = plot_roc_curve(clf, X_test, y_test, ax=ax, name=name)
    except:
        print('Second attempt at roc plot failed for model {0}.\n'.format(name))
    ...
    results.append(
       # [name, clf.score(X_train, y_train), clf.score(X_test, y_test), report]
        [name, disp.confusion_matrix[0,1], disp.confusion_matrix[1,1], report]
    )
...
for model in results:
    plt.plot(model[1], model[2], label=model[0])
