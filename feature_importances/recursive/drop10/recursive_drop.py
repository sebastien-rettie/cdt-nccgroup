...
for name in names:
    ...
    #Number of droppable features. Divide by 10 and floor, then *10
    #This is because we drop 10 least important features each time
    #If total features not divisible by 10, we leave the remainder at the end
    #This is the reason for floor before re-multiply
    
    len_droppable = 10*np.floor(len(feature_list)/10)

    #Column added for each 10 features dropped
    df_results = np.zeros((6, int(len_droppable/10)))
    ...
    while len(dropped_features) != len_droppable:
        ...
        dropped_features += [i[0] for i in imp_results[:10]] #Add names of 10 least important features to drop list
