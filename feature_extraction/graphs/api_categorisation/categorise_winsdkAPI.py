"""
Clone github.com/MicrosoftDocs/sdk-api then run on sdk-api/sdk-api-src/content/
"""
    ...
    for path, subdirs, files in os.walk(target_dir):
        for name in files:
            if os.path.splitext(name)[1] == '.md':
                ...
                if 'function' not in title_line and 'macro' in title_line:
                    continue
...
def main(target_dir):
    ...
    with open('win32API_categories_functionheaders.yaml', 'w') as f:
